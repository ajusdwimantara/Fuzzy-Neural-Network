...
float r(float k){
    return 0.5*sin(0.007*k) + 2*cos(0.059*k);
}
...
int main(){
    ...
    for(int epoch=0;epoch<EPOCH;epoch++){
        for(int k=0;k<TIME;k++){
            ...
            float y_m_res = 0.99*y_m.array[k] + 0.01*r(k);
            ...
            if(u_delta.size > 1){
                ...
                float x2_res = x2(x_1.array[k], x_2.array[k], u_delta.array[k]);
                insertArrayFloat(&x_2, x2_res);

                float x1_res = x1(x_1.array[k], x_2.array[k], u_delta.array[k]);
                insertArrayFloat(&x_1, x1_res);

                float y_delta_res = x_1.array[k+1];
