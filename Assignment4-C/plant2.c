...
#define TIME 1500
#define EPOCH 1000
#define LEARNING_RATE_END 0.00001

// parameter jizz: EPOCH 1000, LEARNING RATE 0.0001, LEARNING RATE END 0.00001 
// parameter tanpa epoch: EPOCH 1, LEARNING RATE 0.03, LEARNING RATE END 0.003
...
float f(float u_c){
    return 0.6*sin(M_PI*u_c) + 0.3*sin(3*M_PI*u_c) + 0.1*sin(5*M_PI*u_c);
}

float u_c(float k){
    return sin((2*M_PI*k)/250);
}
...
int main(){
    ...
    for(int epoch=0;epoch<EPOCH;epoch++){
        for(int k=0;k<TIME;k++){
            ...
            if(y_m.size > 1){
                float y_m_res = 0.3*y_m.array[k] + 0.6*y_m.array[k-1] + f(u_c(k));
                insertArrayFloat(&y_m, y_m_res);
            }
            else{
                float y_m_res = 0.3*y_m.array[k] + f(u_c(k));
                insertArrayFloat(&y_m, y_m_res);
            }
            if(k != 0){
                prev_e = e;
            ...
            if(u_delta.size > 1){
                ...
                float y_delta_res = 0.3*y_delta.array[k] + 0.6*y_delta.array[k-1] + u_delta.array[k];
                insertArrayFloat(&y_delta, y_delta_res);
            }
            else{
                float y_delta_res = 0.3*y_delta.array[k] + u_delta.array[k];
                insertArrayFloat(&y_delta, y_delta_res);
            }
    ...
    wchar_t title[] = L"NN 2";
    DrawScatterPlot(imageRef, 600, 400, x_plot, TIME, y_plot, TIME, x_plot2, TIME, y_plot2, TIME, title);

    size_t length;
    double *pngData = ConvertToPNG(&length, imageRef->image);
    WriteToFile(pngData, length, "NN2.png");
